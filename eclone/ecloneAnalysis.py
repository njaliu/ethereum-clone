...
def analyzeBlock(block):
    ...
    for instr in block_ins:
    	...
    	if instr_type == STORAGE_OP_TYPE:
    		storage_addr = instr.get_storage_access()["storage"]
    		...
    		if mnemonic == 'SSTORE':
    			...
    			if reversed(queue)[0] == 'DEF': # aliu: count a single def
    		...
    		if mnemonic == 'SLOAD':
    			...
    			if reversed(queue)[0] == 'USE': # aliu: count a single use
    ...
    birth_mark = BirthMark(pc_cantor, def_cantor, use_cantor, du_cantor, uu_cantor, update_call, use_call, call_finalize)
...
def block_similarity(block1, block2):
	...
	sim_mark = vector_similarity(birth_mark1, birth_mark2)
	meta_data1 = block1["metadata"]
	meta_data2 = block2["metadata"]
	sim_meta = vector_similarity(meta_data1, meta_data2)
