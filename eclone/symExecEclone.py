...
class Parameter:
    def __init__(self, **kwargs):
        attr_defaults = {
            "instr": "",
            "instr_id": -1, # aliu: id of the current instruction in the current block
            "storage_access": [], # aliu: storage accessed by the current instruction
            "block": 0,
            "depth": 0,
            "pre_block": 0,
            "func_call": -1,
...
def construct_bb():
    ...
    for key in end_ins_dict:
        ...
        if key not in instructions:
            continue
        # aliu: extend instruction with a storage field
...
def sym_exec_block(params):
    ...
    for instr in block_ins:
        #params.instr = instr
        # aliu
        params.instr = instr.get_inst_str()
        params.instr_id = block_ins.index(instr)
        #params.instr = instr["inst_str"]
        #params.instr_id = block_ins.index(instr)
        #params.storage_access = instr["storage"]
        sym_exec_ins(params)
...
def sym_exec_ins(params):
    ...
    #
        ...
        if len(stack) > 0:
            global_state["pc"] = global_state["pc"] + 1
            address = stack.pop(0)
            #log.info("++ visiting SLOAD: " + address)
            
            # aliu: update the storage load
            #storage_load.append(address)
            current_instr = vertices[start].get_instruction(params.instr_id)
            current_instr.set_storage_access(address, "LOAD")
            vertices[start].set_instruction(params.instr_id, current_instr)
            #new_instr_storage = current_instr["storage"]
            #new_instr_storage.append(address)
            #new_instr = {"inst_str": instr, "storage": new_instr_storage}
            #vertices[start].set_instruction(params.instr_id, new_instr)
        ...
        if len(stack) > 1:
            ...
            # aliu: update the storage store
            #storage_store.append(stored_address) 
            current_instr = vertices[start].get_instruction(params.instr_id)
            current_instr.set_storage_access(stored_address, "STORE")
            vertices[start].set_instruction(params.instr_id, current_instr)
            #new_instr_storage = current_instr["storage"]
            #new_instr_storage.append(stored_address)
            #new_instr = {"inst_str": instr, "storage": new_instr_storage}
            #vertices[start].set_instruction(params.instr_id, new_instr)
    ...
    #
        ...
        # aliu: record the current function
        if position == 4:
            #six.print_("hahahahah")
            global current_function
            current_function = instr_parts[1]
            #six.print_(instr_parts[1])
    ...
    #
        ...
        if len(stack) > 6:
            ...
            # aliu: update the CALL recipient
            current_instr = vertices[start].get_instruction(params.instr_id)
            current_instr.set_recipient(recipient)
            vertices[start].set_instruction(params.instr_id, current_instr)
